...
async def find(update, context) -> None:
    msg = update.message.text
    in_info = msg.split()
    await update.message.reply_text(model.find_last_name(in_info[1]))


async def import_txt(update: Update, context: ContextTypes) -> None:
    model.import_into_csv()


app = ApplicationBuilder().token(
    file_token.token()).build()  # Вместо "file_token.token()" необходимо вставить токен телегам-бота
